...
function FILTER_GLOBAL_BLACKLIST_AND_WATCHED_AND_SKIPPED( wNewIDS ) {
	return new Promise( async function( resolve , reject ) {
		try {
			let temp_skipped_key = "TMP_KEY.SKIPPED." + Math.random().toString(36).substring(7);
			let temp_blacklist_key = "TMP_KEY.BLACKLIST." + Math.random().toString(36).substring(7);
			let temp_watched_key = "TMP_KEY.WATCHED." + Math.random().toString(36).substring(7);
