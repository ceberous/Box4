...
function wGetWindowIDFromName( wName ) {
	try {
		...
		if ( wWindowID.stderr.length > 1 ) { Reporter.log( "ERROR --> Could not Wrap FF Window" ); return null; }
		var wF = wWindowID.stdout.trim();
		wF = wF.split("\n");
		wF = wF.pop();
		//if ( wF.length < 1 ) { return null; }
		Reporter.log( "WindowID = " + wF );
		return wF;
	}
...
function  wEnsureWindowNameIsReady( wName ) {
	return new Promise( async function( resolve , reject ) {
		try{
			...
			Reporter.log( "X-Window READY !!! " + xFoundID );
			resolve( xFoundID );
		}
		catch( error ){ Reporter.log( error ); resolve( error ); }
	});
}
...
function wActivateWindowID( wID ) {
	...
	if ( wActivate.stderr.length > 1 ) { Reporter.log( "ERROR --> Could not Activate Window ID" ); return null; }
	else { return true; }
}
...
function wSetWindowIDFocus( wID ) {
	...
	if ( wFocus.stderr.length > 1 ) { Reporter.log( "ERROR --> Could not Focus Window ID" ); return null; }
	Reporter.log( "window activated" );
	return true;
}
...
function wWindowMove( wID , wScreenNum ) {
	...
	if ( wExec1.stderr.length > 1 ) { Reporter.log( "ERROR --> Could not Move Window ID" ); Reporter.log( wExec1.stderr );return null; }
	else { return true; }
}
...
function wSetWindowIDFullScreen( wID , wScreenNum ) {
	...
	if ( wSetFull.stderr.length > 1 ) { Reporter.log( "ERROR --> Could not set Window ID to Full Screen" ); Reporter.log( wSetFull.stderr ); return null; }
	else { return true; }
}
